...
public class Vue_joueur extends JPanel implements AutreEventListener{
	...
	public void lecture_joueur(){
		try {
			...
			modele.setPrintWriter(new PrintWriter(modele.getSocket().getOutputStream()));
			modele.getPrintWriter().println("NumeroJoueur") ;
			modele.getPrintWriter().flush() ;
	...
	public void envoi(){
		try {
			...
			if ((l.getSelectedIndex()==0)||(l.getSelectedIndex()==-1)){
				...
				modele.getPrintWriter().println(env);
				modele.getPrintWriter().flush();
			}else{
...
/// THREAD RECEPTION
	class reception implements Runnable{
		...
		try {
			...
			cap_serveur = new BufferedReader(new InputStreamReader(modele.getSocket().getInputStream()));
			String msg = cap_serveur.readLine();
